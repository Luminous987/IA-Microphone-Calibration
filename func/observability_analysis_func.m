...
%% parameters
% 为什么g.edge一个L一个P？对应的J的行，即测量量是y,δs,y,δs分布的，y是每个时刻传感器阵列的TODA
% graph file
...
for n=2:g.M % 这里是for every microphone，i=2,...,N
    ...
    for i=1:size(ss_idx,2)
        k = ss_idx(i);
        G.ss(n).Gi(i,:) = J_G((g.M-1+3)*(k-1)+(n-1),5*(n-2)+1:5*(n-1));% L的行数是N-1,0矩阵的行数是3，得到M-1+3,乘以k-1为了定位到Lk-1，只有第n-1行有值
